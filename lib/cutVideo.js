...
export function renderTextVideo({ audioPath, words, duration, outputPath }) {
  return new Promise((resolve, reject) => {
    ...
    let command = ffmpeg()
      .input(`color=c=0x0B0E11:s=1080x1920:d=${duration}:r=30`)
      .inputFormat('lavfi')
      .input(audioPath);
...
export function cutClip({
  ...
  return new Promise((resolve, reject) => {
    ...
    const filters = [];
    if (vertical) {
      // Full HD vertical (1080x1920) — máxima nitidez em celular. (Tinha
      // voltado pra 720x1280 suspeitando de falta de memória, mas a causa
      // real dos cortes inválidos era outra — ver lib/highlights.js. Não
      // há evidência de que 1080p sozinho tenha causado problema.)
      ...
      if (typeof faceCenterXRatio === 'number') {
        ...
        filters.push(`crop=1080:1920:x='clip(${faceCenterXRatio}*iw-ow/2\\,0\\,iw-ow)':y=0`);
      } else {
