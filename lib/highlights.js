...
export async function findHighlights(segments, { minClips = 3, maxClips = 8, targetDuration = 60 } = {}) {
  ...
  // Vídeos que já são curtos (ex: um Shorts de 20-30s) não têm material
  // pra vários cortes distintos, nem pra bater a duração-alvo cheia —
  // pedir de 3 a 8 trechos de ~60s de um vídeo de 20s é uma exigência
  // impossível de cumprir, e a IA pode simplesmente desistir e devolver
  // zero cortes em vez de tentar. Detecta isso e ajusta o pedido pro
  // tamanho real do vídeo antes de montar o prompt.
  const totalDuration = segments.length > 0 ? segments[segments.length - 1].end : targetDuration;
  const isShortSource = totalDuration <= targetDuration * 1.2;

  const effectiveMaxClips = isShortSource
    ? 1
    : Math.min(maxClips, Math.max(1, Math.floor(totalDuration / (targetDuration * 0.6))));
  const effectiveMinClips = Math.min(minClips, effectiveMaxClips);

  const minDuration = isShortSource
    ? Math.min(Math.max(10, targetDuration - 10), totalDuration)
    : Math.max(10, targetDuration - 10);
  const maxDuration = isShortSource ? totalDuration : targetDuration + 10;

  const durationInstruction = isShortSource
    ? `Esse vídeo já é curto (tem só ${Math.round(totalDuration)}s no total) — não tem como cumprir a duração-alvo de ${targetDuration}s. Nesse caso, use o vídeo inteiro (ou quase inteiro) como 1 corte só: comece no ponto de maior impacto e termine o mais perto possível do fim real do vídeo (~${Math.round(totalDuration)}s). Não invente conteúdo além do que existe na transcrição, e não deixe de devolver esse corte só porque ele é mais curto que o normal.`
...
Identifique entre ${effectiveMinClips} e ${effectiveMaxClips} trecho${effectiveMaxClips > 1 ? 's' : ''} com o maior potencial de viralizar como corte${effectiveMaxClips > 1 ? 's curtos e independentes' : ' curto'}.
...
${
  isShortSource
    ? 'Corte limpo no final: termine no fim de uma frase completa (ou o mais perto possível do fim do vídeo), em vez de cortar no meio de uma palavra.'
